...
export default function EditLocationModal({ isOpen, onClose, location, onUpdate }) {
  ...
  useEffect(() => {
    if (location) {
      setFormData({
        ...
        pricePerHour: location.pricePerHour ? String(location.pricePerHour) : '100',
      });
    }
  }, [location]);
  ...
  const validate = () => {
    const errs = {};
    if (!formData.name.trim()) errs.name = '❌ Parking name is required.';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ submit: err.message || 'Failed to update parking location.' });
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="bg-slate-900 text-white p-6 flex justify-between items-center">
          <div>
            <h3 className="text-xl font-bold">Edit Parking Location</h3>
            <p className="text-xs text-slate-400 mt-0.5">Update location details and pricing</p>
          </div>
